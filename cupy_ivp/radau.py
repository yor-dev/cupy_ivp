...
C = cp.asarray([(4 - S6) / 10, (4 + S6) / 10, 1])
E = cp.asarray([-13 - 7 * S6, -13 + 7 * S6, -1]) / 3
...
class Radau(OdeSolver):
    ...
    def _validate_jac(self, jac, sparsity):
        ...
        if jac is None:
            ...
            if issparse(J):
                ...
                J = cp.asarray(J, dtype=float)

                def jac_wrapped(t, y, _=None):
                    self.njev += 1
                    return cp.asarray(jac(t, y), dtype=float)
    ...
    def _step_impl(self):
        ...
        while not step_accepted:
            ...
            if self.sol is None:
                Z0 = cp.zeros((3, y.shape[0]))
            else:
                Z0 = self.sol(t + h * C).T - y

            scale = atol + cp.abs(y) * rtol
