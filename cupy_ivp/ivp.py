    ...
    **options
    ...
    if args is not None:
        ...
        try:
            ...
            suggestion_tuple = (
                "Supplied 'args' cannot be ucpacked. Please supply `args`"
                f" as a tuple (e.g. `args=({args},)`)"
            )
            raise TypeError(suggestion_tuple) from exp

        def fun(t, x, fun=fun): return fun(t, x, *args)
        jac = options.get('jac')
        if callable(jac):
    ...
    if t_eval is not None:
        ...
        if tf > t0:
            ...
            # Make order of t_eval decreasing to use cp.searchsorted.
            t_eval = t_eval[::-1]
