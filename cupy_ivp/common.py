...
def validate_tol(rtol, atol, n):
    ...
    if cp.any(cp.asarray(rtol < 100 * EPS)):
...
def num_jac(fun, t, y, f, threshold, factor, sparsity=None):
    ...
    approximately equals ``EPS * cp.abs(f)``. It reduces a possibility of a
    ...
    threshold : float
        ...
        ``factor * cp.maximum(cp.abs(y), threshold)``. Typically, the value of
...
def _sparse_num_jac(fun, t, y, f, h, factor, y_scale, structure, groups):
    ...
    max_ind = cp.asarray(abs(diff).argmax(axis=0)).ravel()
    r = cp.arange(n)
    ...
    if cp.any(diff_too_small):
        ...
        max_ind_new = cp.asarray(abs(diff_new).argmax(axis=0)).ravel()
        r = cp.arange(ind.shape[0])
